...
def _py_kde_deriv_scalar(xi, x_obs, y_obs, bw, deriv_order, use_truncation, cutoff):
    j_lo = 0
    j_hi = x_obs.size - 1
    if use_truncation:
...
def _py_gauss_kde_eval(x, x_obs, y_obs, bw, deriv_order,
...
def _py_gauss_kde_all_eval(x, x_obs, y_obs, bw,
                           use_truncation=True, cutoff=DEFAULT_KDE_CUTOFF):
...
def _bisection_root_kde_deriv(x_obs, y_obs, bw, deriv_order, use_truncation,
    ...
    f_left = _py_kde_deriv_scalar(left, x_obs, y_obs, bw, deriv_order, use_truncation, cutoff) - yi
    f_right = _py_kde_deriv_scalar(right, x_obs, y_obs, bw, deriv_order, use_truncation, cutoff) - yi
    ...
    for _ in range(max_iter):
        mid = 0.5 * (left + right)
        f_mid = _py_kde_deriv_scalar(mid, x_obs, y_obs, bw, deriv_order, use_truncation, cutoff) - yi
...
def _py_find_roots_on_grid(x, y_grid, x_obs, y_obs, bw, yi, tol, max_iter,
...
def _py_find_extrema_from_grid(x, d1_grid, x_obs, y_obs, bw, tol, max_iter,
    ...
    x_roots = _py_find_roots_on_grid(x, d1_grid, x_obs, y_obs, bw, 0.0, tol, max_iter,
                                     use_truncation, cutoff)
    if x_roots.size == 0:
    ...
    d2_roots = np.array([
        _py_kde_deriv_scalar(xr, x_obs, y_obs, bw, 2, use_truncation, cutoff)
        for xr in x_roots
    ])
...
# Backend dispatch
#
# The KDE grid evaluation and bisection root finding are the pipeline's hot
# paths. When the compiled ``_spectrum_math_cpp`` extension (nanobind port of
# ``src/spectrum_math_cpp.cpp``) is importable it backs the public API; otherwise
# the pure-Python ``_py_*`` implementations above are used unchanged. The
# ``_py_*`` functions also serve as the golden reference in the test-suite.
#
# Set ``MALDIASSIST_DISABLE_CPP=1`` to force the pure-Python path (used by the
# parity tests to compare both backends within one process).
# ---------------------------------------------------------------------------
import os as _os

try:  # pragma: no cover - presence depends on the build
    from . import _spectrum_math_cpp as _cpp
except Exception:  # pragma: no cover - extension is optional
    _cpp = None

_CPP_DISABLED = _os.environ.get("MALDIASSIST_DISABLE_CPP", "").strip().lower() in {
    "1", "true", "yes", "on",
}
USE_CPP = (_cpp is not None) and not _CPP_DISABLED
...
def kde_deriv_scalar(xi, x_obs, y_obs, bw, deriv_order, use_truncation, cutoff):
    if USE_CPP:
        return _cpp.kde_deriv_scalar(
            float(xi), _ascontig(x_obs), _ascontig(y_obs), float(bw),
            int(deriv_order), bool(use_truncation), float(cutoff))
    return _py_kde_deriv_scalar(xi, x_obs, y_obs, bw, deriv_order,
                                use_truncation, cutoff)
...
def gauss_kde_eval(x, x_obs, y_obs, bw, deriv_order,
    ...
    if USE_CPP:
        ...
        if deriv_order < 0 or deriv_order > 3:
            raise ValueError("deriv_order must be 0, 1, 2, or 3.")
        return _cpp.gauss_kde_eval(
            _ascontig(x), _ascontig(x_obs), _ascontig(y_obs), float(bw),
            int(deriv_order), bool(use_truncation), float(cutoff))
    return _py_gauss_kde_eval(x, x_obs, y_obs, bw, deriv_order,
                              use_truncation, cutoff)
...
def gauss_kde_all_eval(x, x_obs, y_obs, bw,
    ...
    if USE_CPP:
        if bw <= 0 or not np.isfinite(bw):
            raise ValueError("bw must be a finite positive number.")
        return _cpp.gauss_kde_all_eval(
            _ascontig(x), _ascontig(x_obs), _ascontig(y_obs), float(bw),
            bool(use_truncation), float(cutoff))
    return _py_gauss_kde_all_eval(x, x_obs, y_obs, bw, use_truncation, cutoff)


def find_roots_on_grid(x, y_grid, x_obs, y_obs, bw, yi, tol, max_iter,
                       use_truncation, cutoff):
    if USE_CPP:
        return _cpp.find_roots_on_grid(
            _ascontig(x), _ascontig(y_grid), _ascontig(x_obs), _ascontig(y_obs),
            float(bw), float(yi), float(tol), int(max_iter),
            bool(use_truncation), float(cutoff))
    return _py_find_roots_on_grid(x, y_grid, x_obs, y_obs, bw, yi, tol, max_iter,
                                  use_truncation, cutoff)
...
def find_extrema_from_grid(x, d1_grid, x_obs, y_obs, bw, tol, max_iter,
                           use_truncation=None, cutoff=DEFAULT_KDE_CUTOFF):
    if USE_CPP:
        x_obs_c = _ascontig(x_obs)
        if use_truncation is None:
            use_truncation = _is_strictly_increasing(x_obs_c)
        return _cpp.find_extrema_from_grid(
            _ascontig(x), _ascontig(d1_grid), x_obs_c, _ascontig(y_obs),
            float(bw), float(tol), int(max_iter), bool(use_truncation),
            float(cutoff))
    return _py_find_extrema_from_grid(x, d1_grid, x_obs, y_obs, bw, tol, max_iter,
                                      use_truncation, cutoff)
